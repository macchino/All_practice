...
def insert_muntiple_student(request):
    StudetFormSet = modelformset_factory(Students, fields='__all__', extra=3)
    insert_form = StudetFormSet(request.POST or None, request.FILES or None)
    if insert_form.is_valid():
        insert_form.save()
    return render(
        request, 'form_app/insert_muntiple_student.html', context={
            'insert_form':insert_form
        }
    )
